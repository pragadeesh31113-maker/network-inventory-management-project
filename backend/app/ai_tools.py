...
from .database import SessionLocal # Import for internal session creation
...
# Helper to get DB session inside tools since LangChain tools are stateless
def get_db_session():
    db = SessionLocal()
    try:
        yield db
...
# --- EXISTING TOOLS (IMPROVED & DECORATED) ---

@tool
def suggest_available_assets(asset_type: str, count: int) -> List[Dict[str, Any]]:
    """
...
@tool
def get_customer_hierarchy(customer_name_or_id: str) -> Dict[str, Any]:
    """
...
@tool
def troubleshoot_install_issue(issue_description: str) -> str:
    """
    Provides troubleshooting steps for common technician issues.
    This is a "knowledge base" tool.
    """
    # No DB needed for this one, but good practice to keep structure strict if needed later
    kb = {
...
@tool
def list_devices(status: Optional[str] = None, asset_type: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
@tool
def get_splitter_details(splitter_name: str) -> Dict[str, Any]:
    """
...
@tool
def get_technician_tasks(user_id: int) -> List[Dict[str, Any]]:
    """
    Gets a list of PENDING or IN_PROGRESS tasks for the *currently logged-in technician*.
    """
    # NOTE: The LLM might not know the user_id. We might need to handle this by passing context or asking the user.
    # Ideally, user_id should be injected via a context variable, but for tool simplicity we ask for it 
    # OR we rely on the prompt to inject it.
...
@tool
def get_device_details_by_serial(serial_number: str) -> Dict[str, Any]:
    """
...
@tool
def update_asset_status(serial_number: str, new_status: str, user_id: int) -> Dict[str, Any]:
    """
